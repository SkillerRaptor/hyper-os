...
void Painter::DrawText(const Rect& rect, const String& string, TextAlignment textAlignment, const Color& color)
{
    ...
    {
        PutChar(string[i], Point{ rect.GetX() + 6 + i * 7, rect.GetY() + 6 }, color);
    }
}

void Painter::PutChar(uint16_t c, const Point& cursorPosition, const Color& color)
{
    char* bitmap = (char*)font8x8_basic[c];
    uint8_t* screen = (uint8_t*)m_FrameBufferInfo.FramebufferAddress;
    for (size_t x = cursorPosition.GetX(); x < cursorPosition.GetX() + 8; x++)
    {
        uint32_t screenSpaceX = x * (m_FrameBufferInfo.FramebufferBpp / 8);
        for (size_t y = cursorPosition.GetY(); y < cursorPosition.GetY() + 8; y++)
        {
            uint32_t screenSpaceY = y * m_FrameBufferInfo.FramebufferPitch;
