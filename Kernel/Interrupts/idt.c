...
{
   idt_entries[index].offset_low = ((size_t)handler & 0x0000FFFF) >> 0;
   idt_entries[index].selector = KERNEL_CODE_SELECTOR;
   idt_entries[index].ist = 0x0;
   idt_entries[index].attributes = flags;
   idt_entries[index].offset_mid = ((size_t)handler & 0xFFFF0000) >> 16;
   idt_entries[index].offset_high = ((size_t)handler& 0xFFFFFFFF00000000) >> 32;
